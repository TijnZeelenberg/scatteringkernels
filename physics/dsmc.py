...
class DSMC_Simulation:
    ...
    def select_collision_pairs(self, dt:float, cr_max:float):
        """Select collision pairs using the No-Time-Counter (NTC) method from Bird.

        Based on Garcia's approach: the number of candidate pairs per cell
        is proportional to N*(N-1)*cr_max, and each pair is accepted with
        probability cr/cr_max.
        """
        if self.nr_cells is None:
            raise ValueError(
        ...
        volume = self.volume
        Fn = self.N_real / self.N_sim
        n = N / volume # number density in each cell (nr_cells,)

        eta = np.pi * d**3 * n / 6.0 # eta for each cell (nr_cells,)
        Y = (1 / 2) * ((2 - eta) / ((1 - eta) ** 3)) # Y(n) for each cell (nr_cells,)
        A = Y * n # (nr_cells,)
        Cmax = # maximum magnitude of relative velocities for each particle (N_sim,)
        Nc = 0.5 * np.sum(4*np.pi*sigma * A * Cmax)
