        ...
        if number > 1:
            ...
            for isel in range(nsel):
                ...
                if cr / crmax[jcell] > np.random.random():
                    ...
                    cos_th = 1.0 - 2.0 * np.random.random()  # Cosine and sine of
                    sin_th = np.sqrt(1.0 - cos_th**2)  # collision angle theta
...
def mover(p, v, npart, L, mpv, vwall, tau):
    ...
    p_old = np.copy(p)  # Remember original position
    p[:] = p_old[:] + v[:, 0] * tau

    # * Loop over all particles
    strikes = np.array([0, 0])
    delv = np.array([0.0, 0.0])
    xwall = np.array([0.0, L])
    vw = np.array([-vwall, vwall])
    direction = [1, -1]  # Direction of particle leaving wall
    stdev = mpv / np.sqrt(2.0)
