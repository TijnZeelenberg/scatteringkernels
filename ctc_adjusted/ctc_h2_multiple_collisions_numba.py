...
# ---------------------------------------------------------------------------
ncoll   = 40000
savefile = "data/H2H2_collisions_numba_b1_5.npy"
...
def _fij(Xi, Xj):
    ...
    return np.array([-np.cos(phi)*Fijxy,
                     -np.sin(phi)*Fijxy,
                     -np.sin(theta)*Fmag])
...
def _torques(F13, F14, F23, F24, R1, R2, dH2):
    ...
    F13_r = F13 @ R1;  F14_r = F14 @ R1
    F23_r = F23 @ R1;  F24_r = F24 @ R1
    F31_r = -F13 @ R2; F41_r = -F14 @ R2
    F32_r = -F23 @ R2; F42_r = -F24 @ R2
    h = dH2 / 2.0
    M1 = np.array([
...
def _rdot(w, R):
    ...
    wt = np.zeros((3, 3))
    wt[0, 1] = -w[2];  wt[0, 2] =  w[1]
    wt[1, 0] =  w[2];  wt[1, 2] = -w[0]
    wt[2, 0] = -w[1];  wt[2, 1] =  w[0]
    return R @ wt
...
def _run_one(seed):
    ...
    w1 = np.array([s1*np.sqrt(2.0*f11       *Erot1/_I),
                   s2*np.sqrt(2.0*(1.0-f11)*Erot1/_I), 0.0])
    w2 = np.array([s3*np.sqrt(2.0*f21       *Erot2/_I),
                   s4*np.sqrt(2.0*(1.0-f21)*Erot2/_I), 0.0])
    ...
    R1 = _rand_rot_mat()
    R2 = _rand_rot_mat()
    X11 = X1 + R1@X11_0;  X12 = X1 + R1@X12_0
    X21 = X2 + R2@X21_0;  X22 = X2 + R2@X22_0
    ...
    while dr <= 5.0 * _sigma:
        ...
        gx   = V1[0]-V2[0];  gy = V1[1]-V2[1];  gz = V1[2]-V2[2]
        ...
        if step == 0:
            ek1_0 = ek1;  ek2_0 = ek2
            er1_0 = er1;  er2_0 = er2;  erel_0 = erel
        ek1_f = ek1;  ek2_f = ek2
        er1_f = er1;  er2_f = er2;  erel_f = erel

        # --- forces at current positions ---
        F13 = _fij(X11, X21);  F14 = _fij(X11, X22)
        F23 = _fij(X12, X21);  F24 = _fij(X12, X22)
        F1  = F13 + F14 + F23 + F24
        ...
        F13_ = _fij(X11, X21);  F14_ = _fij(X11, X22)
        F23_ = _fij(X12, X21);  F24_ = _fij(X12, X22)
...
if __name__ == "__main__":
    ...
    print(f"Done. Elapsed: {elapsed:.1f} s  ({elapsed/ncoll*1e3:.2f} ms/collision)\n")
    ...
    ax[0].set_xlabel("Etr before [K]"); ax[0].set_ylabel("Etr after [K]")
    ax[0].set_title("Relative translational energy")

    ax[1].scatter(df["Er1"],  df["Er1p"], s=1, alpha=0.3)
    ax[1].set_xlabel("Er1 before [K]"); ax[1].set_ylabel("Er1 after [K]")
    ax[1].set_title("Rotational energy mol 1")

    ax[2].scatter(df["Er2"],  df["Er2p"], s=1, alpha=0.3)
    ax[2].set_xlabel("Er2 before [K]"); ax[2].set_ylabel("Er2 after [K]")
