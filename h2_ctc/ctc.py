...
def _run_one(seed, E_rel_max):
    ...
    E_rel is drawn uniformly over [0, E_rel_max].
...
def _run_chunk(seed_offset, count, E_rel_max):
    """Run *count* collisions starting at seed *seed_offset*, in parallel."""
    results = np.empty((count, 13))
    for i in prange(count):
        results[i] = _run_one(seed_offset + i, E_rel_max)
    return results


def run_all_collisions(ncoll, seed=0, chunk_size=500, E_rel_max=20100.0):
    ...
    ----------
    E_rel_max : float
        Upper cap [K] for the uniform E_rel distribution.
    ...
    with tqdm(total=ncoll, unit="collision") as bar:
        ...
        while offset < ncoll:
            ...
            results[offset : offset + n] = _run_chunk(seed + offset, n, E_rel_max)
            offset += n
            bar.update(n)
    return results
...
if __name__ == "__main__":
    ...
    _ = _run_chunk(0, 1, E_rel_max)
    print("Compilation done.\n")

    t0 = time.time()
    print(f"Running {ncoll} collisions (uniform, E_rel_max={E_rel_max:.0f} K) …")
    raw = run_all_collisions(ncoll, chunk_size=1000, seed=seed, E_rel_max=E_rel_max)
