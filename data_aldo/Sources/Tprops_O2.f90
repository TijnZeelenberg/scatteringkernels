PROGRAM Omega_O2
  USE Precision
  USE Molprops
  USE Random
  USE Setup
  USE CT_Collider
  USE Scattering
  IMPLICIT NONE
  INTEGER::test,Ntest,ncrit
  REAL(dp),DIMENSION(3)::v1,v2,omega1,omega2
  REAL(dp),DIMENSION(3)::vr,vrstar,umed
  REAL(dp)::g,gstar,coschi,sinchi,sinchi2
  REAL(dp)::erot1,erot1_star,erot2,erot2_star
  REAL(dp)::ttrasl,trot,T0,dens0
  REAL(dp)::omega22,omega11,viscosity,diffusion_coefficient
  REAL(dp)::omega22_tax,Viscosity_tax
  REAL(dp)::fbulk,omega_bulk_tax,bulk_viscosity_tax
  !
  ! Thermal conductivity
  !
  REAL(dp)::fx,ifx,x_tax
  REAL(dp)::fy,ify,y_tax
  REAL(dp)::fyy,ifyy,yy_tax
  REAL(dp)::fz,ifz,z_tax
  REAL(dp)::therm_cond_tax
  ! Per-collision energy diagnostics (written to O2O2_collisions.csv)
  REAL(dp)::Etr_in,Etr_out,Erot1_in,Erot2_in,Erot1_out,Erot2_out
  !
  OPEN(unit=1,file='Omega.inp')
  OPEN(unit=2,file='Viscosity_O2.dat')
  OPEN(unit=3,file='Thermal_cond_O2.dat')
  OPEN(Unit=4,file='Diffusion_coeff_O2.dat')
  OPEN(unit=20,file='O2O2_collisions.csv', status='replace', action='write', form='formatted')
  !OPEN(unit=11,file='Scattering.dat')

  WRITE(20,'(A)') 'Etr,Erot1_in,Erot2_in,Etr_out,Erot1_out,Erot2_out'
  !
  READ(1,*)T0
  READ(1,*)P0
  READ(1,*)Ntest
  READ(1,*)seme
  READ(1,*)h
  !
  dens0=P0/(kb*T0)
  !
  ttrasl=0.0D0;trot=0.0D0;umed=0.0D0
  omega22=0.0D0;omega11=0.0D0;omega22_tax=0.0D0
  omega_bulk_tax=0.0D0
  ifx=0.0D0;ify=0.0D0;ifyy=0.0D0;ifz=0.0D0
  !
  ncrit=0
  !
  DO test=1,Ntest
   !  write(*,*) test
     CALL Maxwell(v1(1),v1(2),v1(3))
     v1=v1*SQRT(Rgas(2)*T0)
     CALL Maxwell(v2(1),v2(2),v2(3))
     v2=v2*SQRT(Rgas(2)*T0)
     CALL Genk(omega1(1),omega1(2),omega1(3))
     erot=-kb*T0*LOG(1.0D0-rf(seme))
     omega1=omega1*SQRT(2.0D0*erot/momin(2))
     erot1=erot
     CALL Genk(omega2(1),omega2(2),omega2(3))
     erot=-kb*T0*LOG(1.0D0-rf(seme))
     omega2=omega2*SQRT(2.0D0*erot/momin(2))
     erot2=erot
     umed=umed+v1+v2
     ttrasl=ttrasl+0.5D0*molmass(2)*Dot_PRODUCT(v1,v1)
     ttrasl=ttrasl+0.5D0*molmass(2)*Dot_PRODUCT(v2,v2)
     trot=trot+0.5D0*momin(2)*Dot_PRODUCT(omega1,omega1)
     trot=trot+0.5D0*momin(2)*Dot_PRODUCT(omega2,omega2)
     vr=v2-v1
     g=SQRT(Dot_PRODUCT(vr,vr))

      ! Energies BEFORE collision
      Etr_in=0.5D0*molmass(2)*Dot_PRODUCT(v1,v1)+0.5D0*molmass(2)*Dot_PRODUCT(v2,v2)
      Erot1_in=0.5D0*momin(2)*Dot_PRODUCT(omega1,omega1)
      Erot2_in=0.5D0*momin(2)*Dot_PRODUCT(omega2,omega2)

     CALL Collision(v1,omega1,v2,omega2,2,2)
     vrstar=v2-v1
     gstar=SQRT(Dot_PRODUCT(vrstar,vrstar))
     IF(ISNAN(gstar).EQV..TRUE.) THEN
        WRITE(118,*)'Collisione critica'
        ncrit=ncrit+1
        GO TO 111
     END IF

      ! Energies AFTER collision
      Etr_out=0.5D0*molmass(2)*Dot_PRODUCT(v1,v1)+0.5D0*molmass(2)*Dot_PRODUCT(v2,v2)
      Erot1_out=0.5D0*momin(2)*Dot_PRODUCT(omega1,omega1)
      Erot2_out=0.5D0*momin(2)*Dot_PRODUCT(omega2,omega2)

      WRITE(20,300) Etr_in,Erot1_in,Erot2_in,Etr_out,Erot1_out,Erot2_out
     
     
     coschi=Dot_PRODUCT(vr,vrstar)/(g*gstar)
     sinchi2=1.0D0-coschi**2
     erot1_star=0.5D0*momin(2)*Dot_PRODUCT(omega1,omega1)
     erot2_star=0.5D0*momin(2)*Dot_PRODUCT(omega2,omega2)
     omega22=omega22+(1.0D0-coschi**2)*g**5
     omega11=omega11+(1.0D0-coschi)*g**3
     omega22_tax=omega22_tax+(sinchi2*g**4-(2.0D0/3.0D0*gstar**2+sinchi2*g**2)*(g**2-gstar**2))*g
     !
     ! volume viscosity
     !
     fbulk=(3.0D0*erot1_star-3.0D0*kb*T0/2.0D0-molmass(2)*g**2/4.0D0)*(g**2-gstar**2)*molmass(2)/4.0D0
     fbulk=fbulk+((g**2-gstar**2)*molmass(2)/4.0D0)**2
     fbulk=fbulk*g
     omega_bulk_tax=omega_bulk_tax+fbulk
     !
     ! thermal conductivity
     !
     fx=(molmass(2)/4.0D0*g**2)**2*sinchi2
     fx=fx+(molmass(2)/4.0D0*g**2*coschi**2+25.0D0/8.0D0*kb*T0-15.0D0/4.0D0*molmass(2)/4.0D0*g**2)*&
          molmass(2)/4.0D0*(g**2-gstar**2)+11.0D0/4.0D0*(molmass(2)/4.0D0*(g**2-gstar**2))**2
     fx=fx*g
     ifx=ifx+fx
     !
     fy=(3.0D0/2.0D0*kb*T0-molmass(2)/4.0D0*g**2)*molmass(2)/4.0D0*(g**2-gstar**2)
     fy=fy+(molmass(2)/4.0D0*(g**2-gstar**2))**2
     fy=fy*g
     ify=ify+fy
     !
     fyy=(erot1_star-kb*T0)*molmass(2)/4.0D0*(g**2-gstar**2)*g
     ifyy=ifyy+fyy
     !
     fz=3.0D0/2.0D0*kb*T0*(g**2-gstar**2)+gstar**2*(erot1_star-erot2_star)-g*gstar*(erot1-erot2)*coschi
     fz=fz*(erot1_star-kb*T0)*g
     ifz=ifz+fz
111  CONTINUE
  END DO
  Ntest=Ntest-ncrit
  umed=umed/(2.0D0*Ntest)
  ttrasl=ttrasl/(2.0D0*Ntest)-0.5D0*molmass(2)*Dot_PRODUCT(umed,umed)
  ttrasl=2.0D0*ttrasl/(3.0D0*kb)
  trot=trot/(2.0D0*Ntest)
  trot=trot/kb
  Omega22=Omega22/Ntest
  Omega22=Omega22*(pi/4.0D0)*(1.0D0/(4.0D0*Rgas(2)*T0)**2)*diamol(2,2)**2/2.0D0
  Viscosity=5.0*kb*T0/(8.0D0*omega22)
  Omega22_tax=Omega22_tax/Ntest
  Omega22_tax=Omega22_tax*(pi/4.0D0)*(1.0D0/(4.0D0*Rgas(2)*T0)**2)*diamol(2,2)**2/2.0D0
  Viscosity_tax=5.0*kb*T0/(8.0D0*omega22_tax)
  !
  omega_bulk_tax=omega_bulk_tax/Ntest
  omega_bulk_tax=omega_bulk_tax*pi*diamol(2,2)**2
  omega_bulk_tax=omega_bulk_tax*SQRT(pi)/(4.0D0*SQRT(4.0D0*Rgas(2)*T0)*(kb*T0)**2) !ok
  bulk_viscosity_tax=SQRT(pi*molmass(2)*kb*T0)/(10.0D0*omega_bulk_tax)
  !
  Omega11=Omega11/Ntest
  Omega11=Omega11*pi/(16.0D0*Rgas(2)*T0)*diamol(2,2)**2/2.0D0
  Diffusion_Coefficient=3.0*kb*T0/(8.0D0*molmass(2)*dens0*Omega11)
  WRITE(4,100)T0,Diffusion_Coefficient,Omega11
  WRITE(2,100)T0,Ttrasl,Trot,Omega22,Viscosity,Omega22_tax,Viscosity_tax,omega_bulk_tax,bulk_viscosity_tax
  !
  ! Thermal conductivity
  !
  ifx=ifx/Ntest;ifx=ifx*pi*diamol(2,2)**2
  x_tax=ifx*SQRT(pi)/(8.0D0*SQRT(Rgas(2)*T0)*(kb*T0)**2)
  x_tax=x_tax*4.0D0*SQRT(Rgas(2)*T0/pi)
  !
  ify=ify/Ntest;ify=ify*pi*diamol(2,2)**2
  y_tax=ify*SQRT(pi)/(8.0D0*SQRT(Rgas(2)*T0)*(kb*T0)**2)
  y_tax=y_tax*(-5.0D0)*SQRT(Rgas(2)*T0/pi)
  !
  ifyy=ifyy/Ntest;ifyy=ifyy*pi*diamol(2,2)**2
  yy_tax=ifyy*SQRT(pi)/(8.0D0*SQRT(Rgas(2)*T0)*(kb*T0)**2)
  yy_tax=yy_tax*(-10.0D0)*SQRT(Rgas(2)*T0/pi)
  !
  ifz=ifz/Ntest;ifz=ifz*pi*diamol(2,2)**2
  z_tax=ifz*SQRT(pi*Rgas(2)*T0)/(32.0D0*(Rgas(2)*T0)**2*(kb*T0)**2)
  z_tax=z_tax*(4.0D0)*SQRT(Rgas(2)*T0/pi)
  !
  therm_cond_tax=3.0D0*kb**2*T0/(2.0D0*molmass(2))*(x_tax-5.0D0*y_tax/2.0D0-5.0D0*yy_tax/2.0D0+25.0D0/4.0D0*z_tax)
  therm_cond_tax=therm_cond_tax/(x_tax*z_tax-y_tax*yy_tax)
  WRITE(3,100)T0,x_tax,y_tax,yy_tax,z_tax,therm_cond_tax
100 FORMAT(10(E13.5,2X))
  300 FORMAT(ES24.16,',',ES24.16,',',ES24.16,',',ES24.16,',',ES24.16,',',ES24.16)
END PROGRAM Omega_O2
