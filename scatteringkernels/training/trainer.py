...
for dataset in DATASETS:
    ...
    # Train the model
    train_loader, val_loader = model.create_dataloaders(X, y, batch_size=config.batch_size, shuffle=config.shuffle, trainval_split=config.trainval_split, random_seed=config.random_seed)
    train_loss_history, val_loss_history = model.train_model(train_loader, val_loader, optimizer, num_epochs=config.num_epochs, lr=config.learning_rate)
    ...
    plt.xlabel('Epoch', fontsize=plottingconfig.label_fontsize, fontweight=plottingconfig.label_fontweight)
    plt.ylabel('Loss', fontsize=plottingconfig.label_fontsize, fontweight=plottingconfig.label_fontweight)
