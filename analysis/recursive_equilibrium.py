"""The wrong DSMC equilibrium is the MDN kernel's learned detailed-balance violation.
...
# --------------------------------------------------------------------------- #
# A run is treated as settled if T_trans drifts less than this over the averaging
# window; otherwise the last-N%-mean equilibrium estimate is contaminated by the
# still-relaxing transient.
...
def dsmc_converged_temperatures(npy_path):
    ...
    (mean over the last 20 % of steps).  The window mean is only meaningful once
    the curve has plateaued; the returned drift (and a printed warning) flags a
    run that is still relaxing faster than CONVERGENCE_TOL_K_PER_NS."""
    a = np.load(npy_path)
    k = max(1, len(a) // 5)
    ...
    if k >= 2:
        ...
        slope = float(np.polyfit(t_ns, a["T_trans_mean"][-k:], 1)[0])  # K per ns
        if abs(slope) > CONVERGENCE_TOL_K_PER_NS:
            print(
                f"  ⚠ NOT CONVERGED: T_trans still drifting {slope:+.2f} K/ns over the "
...
def db_violation(Tt_grid, drift, window=30.0):
    ...
    a +/- `window` K band around equipartition -- robust to Monte-Carlo noise and
    self-consistent with the plotted curve and its zero-crossing):
...
# --------------------------------------------------------------------------- #
# The figure: one row, two panels (H2 | O2)
# --------------------------------------------------------------------------- #
def main(output_path: str | None = None):
    ...
    header = (f"{'':5}{'D_eq':>8}{'S':>9}{'-D_eq/S':>9}"
    ...
    for ax, (title, ctc_file, model_path, dsmc_npy) in zip(axes, CASES):
        ...
        # --- plot: D(T_trans) along the conservation line ---
        # The red curve is identified by the y-axis; the legend is reserved for
        # the three temperatures being compared (equipartition, kernel T*, DSMC).
        ax.axhline(0.0, color="0.5", lw=1.0)
        ax.plot(Tt_grid, drift, "o-", color="tab:red", lw=1.8, ms=4)

        ax.axvline(T_EQUIPART, color="0.4", ls=":", lw=1.4,
                   label=fr"equipartition {T_EQUIPART:.0f} K")
        ax.axvline(Tt_dsmc, color="black", ls="--", lw=1.6,
                   label=fr"DSMC converged {Tt_dsmc:.0f} K")

        # focus on the physics around equipartition / the zero-crossing
    ...
    # Drop a copy next to the other thesis figures, matching detailed_balance.py.
    thesis_dir = "../Master_Thesis_Tijn_Zeelenberg/figures"
    if os.path.isdir(thesis_dir):
        fig.savefig(f"{thesis_dir}/recursive_equilibrium.png", dpi=300)
        print(f"Saved {thesis_dir}/recursive_equilibrium.png")
