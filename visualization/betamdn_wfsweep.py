...
# --- simulation parameters (identical to betamdn_H2_energy_relaxation.py) ---
randomseed = 42
box_size = 7.5e-6
volume = box_size**3
dt = 1e-5
...
for i, wf in tqdm(
    ...
    beta_mdn.load_model(
        f"results/models/beta_mdn/weightsensitivity/H2_400000/beta_mdn_H2_wf{str(wf).replace('.', '_')}.pth"
    )
    ...
    sim.run_simulation(nr_steps=nr_steps, dt=dt, collision_model=beta_mdn)
    stats = sim.get_stats()

    ax.plot(stats["timestep"], stats["T_trans_mean"], label="$T_{trans}$ Beta MDN")
    ax.plot(stats["timestep"], stats["T_rot_mean"], label="$T_{rot}$ Beta MDN")
    ax.plot(
        t_spartaVHS,
        T_trans_spartaVHS,
        linestyle="--",
...
fig.suptitle(
    f"H2 Energy Relaxation — Beta MDN Weighting Factor Sweep - Randomseed {randomseed}",
    fontsize=16,
    fontweight="bold",
)
fig.tight_layout()
fig.savefig(
    f"results/models/beta_mdn/weightsensitivity/H2_400000/H2_betamdn_wfsweep.png",
    dpi=300,
)
plt.show()
